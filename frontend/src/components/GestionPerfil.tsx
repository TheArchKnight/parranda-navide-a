...
import PasswordStrengthChecker from "../components/formComponents/PasswordStrengthChecker";
import ConfirmPasswordChecker from "../components/formComponents/ConfirmPasswordChecker";
import PasswordInput from "../components/formComponents/PasswordInput";
import profileImagen from "../assets/images/perfil-defecto.png";
...
const GestionPerfil: React.FC = () => {
  ...
  const [profileImage, setProfileImage] = React.useState(user?.photo );
  ...
  const handleChangePhoto = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Actualizar la foto de perfil en el frontend
    ...
    <> 
      <div className="flex flex-col items-center min-h-screen bg-gray-100 p-6">
        <div className="flex items-center gap-2 font-semibold ml-auto">
          <h2 className="bg-red-500 text-white text-[25px] px-10 py-1 rounded-full">
            Perfil
          </h2>
        </div>
        {/* seccion principal */}
...
<div className="w-full md:w-1/3 flex flex-col justify-center items-center bg-red-500 p-6 min-h-full">
  <div className="w-24 h-24 rounded-full border-4 border-white shadow-md overflow-hidden">
    <img src={profileImagen} alt="Foto de perfil" className="w-full h-full object-cover" />
  </div>

  <input type="file" id="upload-photo" className="hidden" onChange={handleChangePhoto} accept="image/*" />

  <label htmlFor="upload-photo" className="mt-4 w-full">
    <Button className="w-full bg-white hover:bg-gray-400">
      <p className="text-red-500">Cambiar Foto</p>
    </Button>
  </label>
</div>
